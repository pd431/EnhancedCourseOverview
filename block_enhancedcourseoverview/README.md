...
This plugin defines its own webservice, `block_enhancedcourseoverview_get_courses_with_roles` (see `classes/external/get_courses_with_roles.php`), to fetch the user's courses for the current view in the background - without touching or forcing anything to render. It only ever returns the logged-in user's own courses and own roles - there's no way to query another user's data through it.

For `favourites`, `hidden`, and `customfield` classifications, it still calls Moodle's own `core_course_external::get_enrolled_courses_by_timeline_classification()` directly (the exact function `block_myoverview` itself uses) and only adds role data to its output - the same approach Moodle core itself uses in `core_course\external\get_enrolled_courses_with_action_events_by_timeline_classification` to add calendar event data - so this plugin stays riding on core's own favourites/hidden-course logic instead of forking it. (It deliberately requests core's default, full course field set rather than restricting `$requiredfields` to just `id`/`fullname`: on at least one tested Moodle version, restricting it makes core's own return-value validation throw a coding exception, because core still needs a field like `enddate` internally to compute the classification but no longer declares it in the narrowed structure it validates its own output against.)

For the common `all`/`inprogress`/`past`/`future` classifications, it avoids that core function entirely. It's built for *rendering* the dashboard, so every course it returns gets decorated with a completion progress percentage, card image, and other display data - each requiring its own per-course queries - none of which this plugin needs, since it only ever wants a course's id, fullname, and the user's role(s), to decide which filter buttons should be visible. This is a well-documented Moodle performance issue (see [MDL-72246](https://tracker.moodle.org/browse/MDL-72246), fixed for its specific cause in Moodle 3.11.6 but liable to recur for other decorative fields on a site with many completion-tracked courses per user): for a user enrolled in hundreds of courses, that per-course decoration can dominate the load time, even though the underlying enrolment query itself is fast. Instead, the fast path uses `enrol_get_all_users_courses()` - the same plain, single-query enrolment lookup many other core areas use, with no per-course decoration at all - and classifies each course into a bucket itself from its start/end dates (a courses' individually-hidden-from-dashboard status is still respected: since Moodle stores that as an internal preference rather than a public API, this asks core for just its `hidden` bucket to get the excluded ids, which is cheap in practice since almost no one hides more than a handful of courses, unlike `all`). One known trade-off: unlike core's own function, this fast path doesn't check the `moodle/course:viewhiddencourses` capability against `visible = 0` courses - a course hidden by its own teacher could appear in this webservice's data for a user who wouldn't actually see it rendered. Since course rendering itself is always still done by `block_myoverview`'s own pipeline (never bypassed), the only consequence is a filter button that's visible when it shouldn't be (or vice versa) until the next background refresh corrects it - never incorrect course access.

Roles are fetched for every course in one batched pair of queries, covering direct course-level role assignments plus anything inherited from a course's category (or an ancestor category) or the system level, replicating `get_user_roles()`'s own default behaviour - rather than one `get_user_roles()` call per course, which was a second, self-inflicted N+1 query pattern on top of whatever the course list itself cost.
...
Whenever a filter is already known to be active before anything has loaded - from that cache, or a server-rendered `|default` - the actual course list (not just the filter bar) is hidden the same way until filtering has actually been applied to it, so the full unfiltered list never flashes visible for a moment before narrowing down. With no such filter active, the course list is left alone entirely, exactly as `block_myoverview` would render it unmodified - this only ever engages when a filter is already known to apply.
